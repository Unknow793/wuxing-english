...
- **移除测试按钮**：删除「解锁全部头像框」测试按钮及对应函数 unlockAllFrames() 和 CSS
- **文档化头像框系统**：在持久记忆中完整记录头像框定义、解锁条件类型、三级动画体系、CSS 类名模式
- **文档化成系统**：在持久记忆中完整记录15项成就、统计计数器、称号自动授予机制

### 头像框概率掉落系统 + 四个双色系

- **修复橙系锁定Bug**：`showFrameSelect` 中 `isUnlocked` 判断改为 `f.id === '' || unlocked.has(f.id)`，修复 condition:null 导致橙系误显已解锁的问题
- **新增12个双色系头像框**：朱翠/青橙/靛黄/黛金 各3阶，均设 condition:null
- **CSS 三套样式**：4系列各添加预览着色、个人页展示、排行榜展示（双色box-shadow环形边框）
- **橙系掉落（1-4年级）**：
  - 学习模式拼对时 6% → 橙·初
  - 练习模式 ≥80%正确率 4% → 橙·极
  - 初级挑战 Boss胜利 2% → 橙·臻
- **朱翠系掉落（5-6年级）**：
  ...
  - 练习模式 ≥80%正确率 4% → 朱·翠·极
  - 中级挑战 Boss胜利 2% → 朱·翠·臻
- **青橙/靛黄/黛金 暂锁**，condition:null，留待后续分配
