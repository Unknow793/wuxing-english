...
### 5-6年级学习模式：拼词热身改为学→拼两轮制

- **5-6年级拼词热身重构**：不再直接拼3个词，改为（学3个→拼1个）×2轮的热身模式
- 每轮先从 intermediate 词库学3个单词（展示emoji分类+英文+中文，含随堂测验排程）
- 学完每批3个后，用该批最后一个词进行拼词
- 两轮完成后自动转入组句子模式
- 词汇范围保持 intermediate 词库不变，与低年级 beginner 区分
- 进度条跨两批显示全局进度（如 1/6 → 6/6）

## 2026-06-14

### 讨论记录：后续三大方向
- **成就体系重构**：当前15项成就零散不成体系，计划整理所有可追踪元素，分类分层设计（战斗/收集/成长/挑战等篇），基于新体系重新设计称号系统（视觉效果+属性加成）
- **图鉴收集独立系统**：按收集率设里程碑奖励（30%/60%/100%），与成就并行，奖励错开
- **每日签到系统**：连续签到1/3/5/7天递进奖励，断签重置，奖励含限定头像框。定为下次更新核心任务

### 5-6年级学习模式：拼词热身 + 造句串联

- **5-6年级学习模式改为两阶段串联**：先拼词热身（3个词），自动转入连句模式
- 拼词阶段选 3 个有分类的词汇，使用 5-6 年级词库（长词偏多，天然增加挑战）
- 拼完3词后显示转场动画「🎯 拼词热身完成！接下来学习如何用单词造句」
- 转场后进入现有连句模式（word → quiz → sentence）
- 拼词阶段同样适用 6% 概率掉落朱·翠·初头像框
- 低年级（1-2/3-4年级）学习模式保持不变

### 单词分类题型加入 5-6 年级练习模式 + 中级挑战模式

- **5-6 年级练习模式**新增 `word-class` 题型
- **中级挑战模式**改为多题型出题（完形填空/中英配对/单词分类随机抽取），不再仅限完形填空
- `generateIntermediateBossQuestion()` 使用 `pracGenQuestion(type, 'grade5-6')` 生成题目
- 多题型题目无 `correctWord` 时，正确仅扣 Boss 10HP 不加手牌，避免 undefined 错误
- 拼词阶段选 3 个有分类的词汇，使用 5-6 年级词库（长词偏多，天然增加挑战）
- 拼完3词后显示转场动画「🎯 拼词热身完成！接下来学习如何用单词造句」
- 转场后进入现有连句模式（word → quiz → sentence）
- 拼词阶段同样适用 6% 概率掉落朱·翠·初头像框
